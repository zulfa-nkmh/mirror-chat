...
import { useChatStore } from "@/store/chatStore"; // Pastikan path benar
import { useUser } from '@/context/UserContext'; // <-- PENTING: Import User Context
import { useRouter } from 'next/navigation'; // <-- PENTING: Import Router Next.js
...
export default function ChatLayout() { 
  ...
  const { userId, isLoggedIn, logout } = useUser(); // <-- Ambil dari User Context
  ...
  const REFUSAL_KEYWORDS = ["susah", "sulit", "berat", "malas", "capek", "gagal"];
  const MENTOR_KEYWORDS = ["mentor", "stuck", "deadlock", "ahli"];

  // 🔹 Efek: Redirect jika belum login
  useEffect(() => {
    // Jika tidak sedang login dan tidak sedang memuat, arahkan ke /login
  ...
  const handleSend = (e) => {
    ...
    setTimeout(() => {
      ...
      if (botResponse === null && activeScenario) {
          ...
          if (REFUSAL_KEYWORDS.some(keyword => normalizedQuery.includes(keyword))) {
              const mirrorText = activeScenario.mirrorResponses?.[0]?.botAnswer || "Aku dengar kamu ragu. Ingatlah bahwa keraguan adalah awal dari pelajaran baru. Apa yang membuatmu merasa tidak bisa?";
              
              botResponse = {
                  role: "bot",
                  content: `🪞 Cermin Refleksi (Penolakan dari Skenario Aktif - ${activeScenario.skillGap}):\n\n${mirrorText}`,
              };
          }

          // B. Cek Normal Response (TINGKAT 1)
          if (botResponse === null) {
              const matchedNormalResponse = activeScenario.normalResponses?.find(
                  (resp) => {
                      const question = resp.userQuestion?.toLowerCase().replace(/[?,.!-]/g, '').trim();
              ...
              if (matchedNormalResponse) {
                  botResponse = {
                      role: "bot",
                      content: `💡 Saran Spesifik (Skenario Aktif - ${activeScenario.skillGap}):\n\n${matchedNormalResponse.botAnswer}`,
                  };
              }
          }
      }
      
      // 3. Fallback Kontekstual (Jika tidak ada match spesifik, tapi skenario aktif ada)
      if (botResponse === null && activeScenario) { 
           botResponse = {
            role: "bot",
            content: `Aku dengar kamu masih fokus pada **${activeScenario.skillGap}**. \n\nBisa ceritakan lebih detail, atau apa lagi yang bisa aku bantu di area ini?`,
          };
      }
      
      // 4. Final Fallback (Jika skenario aktif entah bagaimana hilang)
      if (botResponse === null) {
           botResponse = {
            role: "bot",
